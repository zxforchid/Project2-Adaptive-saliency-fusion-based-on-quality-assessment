...
imwritePath = ['.\data\revised\image\',datasets{1,dd},'\',TOPfile,'\',testName,'\'];
...
parfor itr=1:20
    i = trlist(itr);
     [~,~,~] = extractQualityFeatureM3_forruntime(i, init_infor,Thigh,Tlow, 'test',testName);
end
% t2 = clock;
% deltat_21 = etime(t2,t1);
...
% % 测试训练集中有多少幅显著性图
load ([savePath,'qualityData',datasets{1,dd},'-',testName(5:end),'.mat'])
LLL=0;
for jj=1:numtr
    tmpIndex = trainIndex{jj,1};
    if ~isempty(tmpIndex)
        for ii=1:length(tmpIndex)
            LLL = LLL + tmpIndex(ii);
        end
    end
end
...
for ite = 1:length(telist)
    ...
    outputFileID(ite) = ObtainQualityscoreM3new_forruntime(i, init_infor,testName,'test',lambdaPara);
end
end
