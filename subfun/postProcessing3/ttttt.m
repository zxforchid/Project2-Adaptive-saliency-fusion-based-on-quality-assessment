...
lambdaParas = [0.1];%[0.35,0.4,0.45,0.5,0.55];%[0.15,0.2,0.25,0.3];%[7,15];%[50, 20, 5, 0.6, 0.1];
...
% for dd=1
imwritePath = ['.\data\img_result(new5)\method3\Evalueate\',datasets{1,dd},'\',FeatureName,'\',TOPfile,'\',testName,'\'];
savePath = ['.\data\img_result(new1)\method3\Evalueate\',FeatureName,'\',TOPfile,'\'];
...
for ite = 1:length(telist)
       disp(ite)
       i = telist(ite);
       outputFileID(ite) = posprocessFunM3_31(i, init_infor,testName,'test', lambdaPara);
