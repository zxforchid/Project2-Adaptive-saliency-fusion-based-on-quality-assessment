function outputFileID = ObtainQualityscoreM3(i, init_infor,testName)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
switch dataSet
    case 'MSRA10K'
        load(['.\data\qualitydata\',dataSet,'\test\',imname,'.mat'])
    case 'ECSSD' 
        load(['.\data\qualitydata\',dataSet,'\test\',imname,'.mat'])
    case 'PASCAL850'
        load(['.\data\qualitydata\',dataSet,'\test\',imname,'.mat'])      
...
% 去除NAN
testDataMatrix = [testMat.FC,testMat.FCP,testMat.FH,testMat.FCS,testMat.FNC, ...
    testMat.FB,testMat.DMSV,testMat.SPE,testMat.SV,testMat.IC];
...
clear testDataMatrix
    ...
    testMat.FB(indexNAN,:) = [];
    testMat.DMSV(indexNAN,:) = [];
    testMat.SPE(indexNAN,:) = [];
    testMat.SV(indexNAN,:) = [];
    testMat.IC(indexNAN,:) = [];
...
[testMat.DMSV] = scaleData_test(testMat.DMSV,0, 1, scaleMkb.DMSVmin, scaleMkb.DMSVmax);
[testMat.SPE]  = scaleData_test(testMat.SPE, 0, 1, scaleMkb.SPEmin,  scaleMkb.SPEmax);
[testMat.SV]   = scaleData_test(testMat.SV,  0, 1, scaleMkb.SVmin,   scaleMkb.SVmax);
[testMat.IC]   = scaleData_test(testMat.IC,  0, 1, scaleMkb.ICmin,   scaleMkb.ICmax);
...
smap = uint8(smap);
imwrite(smap, [init_infor.imwritePath imname '_m3.png'])

smap1 = (smap1 - min(smap1(:))) / (max(smap1(:)) - min(smap1(:)) + eps) * 255;
smap1 = uint8(smap1);
imwrite(smap1, [init_infor.imwritePath imname '_m3_1.png'])

smap2 = (smap2 - min(smap2(:))) / (max(smap2(:)) - min(smap2(:)) + eps) * 255;
smap2 = uint8(smap2);
imwrite(smap2, [init_infor.imwritePath imname '_m3_2.png'])
